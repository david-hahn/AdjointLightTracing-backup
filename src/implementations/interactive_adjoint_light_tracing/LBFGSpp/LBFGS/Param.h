...
public:
    ...
    bool removeOldestOnCurvatureFailure, removeNewestOnCurvatureFailure; // default false, if set remove the oldest/newest entry from the history if a line search step failed to satisfy the curvature condition (standard Wolfe)
...
public:
    ...
    LBFGSParam()
    {
        ...
        ftol           = Scalar(1e-4);
        wolfe          = Scalar(0.9);
		init_step	   = Scalar(1.0);
        removeOldestOnCurvatureFailure=false;
        removeNewestOnCurvatureFailure=false;
    }
